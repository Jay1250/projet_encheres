...
public class VerifConnexion extends HttpServlet implements Servlet {
	...
	/**
	 * @see HttpServlet#HttpServlet() servlet appel�e pour v�rifier que la
	 *      session de l'utilisateur n'a pas expir�. Elle est appel�e lors des
	 *      actions suivantes : cas 1: sur la page d'accueil (de connexion), le
	 *      bouton connexion renvoie � cette servlet via une m�thode post avec
	 *      les setParmeter des champs identifiant et mot de passe avant de
	 *      transmettre : - si les deux champs ne sont pas remplis : renvoie � la
	 *      meme page de connexion - si champs remplis, renvoie � la
	 *      servletConnexion qui v�rifie que les pseudos et mdp correspondent �
	 *      un utilisateur dans la base puis renvoie � la servlet session pour
	 *      ouvrir une session avec set Attribut Utilisateur trouve (� ce stade il
	 *      peut �tre null) et d�marrer un compteur qui comptera le nb de
	 *      tentatives effectu�es par l'utilisateur cas 2: sur la barre de
	 *      navigation, le lien "connexion" ou le lien "trocencheres.org" renvoie
	 *      � cette servlet via m�thode get avant de transmettre � la page de
	 *      connexion si la session a expir� ou � la page listeEncheres si
	 *      session active
	 */
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		// variable compteur qui permet de d�tecter cb de fois l'utilisateur a envoye
		// un
...
// si aucun utilisateur "valide" a �t� mont� en session (cas 1 : premiere connexion cas 2 : au moins une tentative infructuseuse (soit mdp incorrect soit champs non remplis) cas 3 : session expir�e)
		if (request.getSession().getAttribute("utilisateurConnecte") == null) {
			...
			// si 1ere tentative, ou suivantes si les pr�c�dentes n'ont pas abouti faute
			// de
			// remplir tous les champs
			else {
				identifiant = identifiant.trim();
				mdp = mdp.trim();
				// si les deux champs ne sont pas remplis: renvoie � la m�me page connexion
				// avec
				// un message d'alerte
			...
			// si un utilisateur valide a été trouvé et/ou que une session utilisateur
			// est
			// déjà active
